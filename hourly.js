...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateDisplay(selectedDateStr) {
        ...
        // DEV NOTE: In the hourly dataset the temperature key for "now" may differ
        // (e.g. 'temp_now', 'target_temp_hour+0' or similar). If hourly_rich_data.json
        // uses a different key, update the string below accordingly.
        ...
        for (let h = 1; h <= 5; h++) {
            // DEV NOTE: For hourly_rich_data.json the horizon keys may use a different naming
            // convention (for example 'target_temp_h+${h}', 'temp_hour+${h}', 'icon_hour+${h}', etc.).
            // Update these accesses to match the actual hourly data structure when available.
